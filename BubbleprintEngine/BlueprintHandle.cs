...
public static class JsonExtensions
{
    ...
    public static BlueprintHandle DeRef(this JsonElement elem, BlueprintDB db)
    {
        if (!elem.TryDeRef(db, out var bp))
            throw new Exception("could not derefence blueprint from json element: " + elem);
        return bp;
    }
    public static IEnumerable<BlueprintHandle> EnumerateReferences(this JsonElement elem, BlueprintDB db, string child = null)
    {
        if (child != null)
            elem = elem.GetProperty(child);
        foreach (var id in elem.EnumerateArray())
        {
            if (id.TryDeRef(db, out var res))
                yield return res;
        }
    }
    ...
    public static bool TryDeRef(this JsonElement elem, BlueprintDB db, out BlueprintHandle bp, params string[] path)
    {
        var child = elem.Find(path);
        return child.TryDeRef(db, out bp);
    }
