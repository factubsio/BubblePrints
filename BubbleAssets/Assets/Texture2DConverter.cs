    ...
    public class Texture2DConverter
    {
        ...
        private bool DecodeRHalf(byte[] image_data, byte[] buff)
        {
            for (var i = 0; i < outPutSize; i += 4)
            {
                buff[i] = 0;
                buff[i + 1] = 0;
                buff[i + 2] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i / 2) * 255.0f);
                buff[i + 3] = 255;
            }
            return true;
        }
        ...
        private bool DecodeRGHalf(byte[] image_data, byte[] buff)
        {
            ...
            {
                buff[i] = 0;
                buff[i + 1] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i + 2) * 255f);
                buff[i + 2] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i) * 255f);
                buff[i + 3] = 255;
            }
            return true;
        }
        ...
        private bool DecodeRGBAHalf(byte[] image_data, byte[] buff)
        {
            for (var i = 0; i < outPutSize; i += 4)
            {
                buff[i] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i * 2 + 4) * 255f);
                buff[i + 1] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i * 2 + 2) * 255f);
                buff[i + 2] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i * 2) * 255f);
                buff[i + 3] = (byte)Math.Round((float)BitConverter.ToHalf(image_data, i * 2 + 6) * 255f);
            }
            return true;
        }
